...
module.exports = function(ovt) {
  ...
  ovt.registerLocale('en', {
    string: {
      ...
      isByteLength: 'length must be less than {{0.max}} and longer than {{0.min}}',
      isCreditCard: 'is not a valid credit card',
      isCurrency: 'is not a valid currency amount',
      isDataURI: 'is not a valid data uri',
      ...
      isIn: 'must be one of "{{args}}"',
      isInt: 'is not a valid integer',
      isJSON: 'is not valid json',
      isLength: 'length must be less than {{0.max}} and longer than {{0.min}}',
  ...
  ovt.registerLocale('zh-CN', {
    string: {
      ...
      isByteLength: '长度应大于{{0.min}}个字节, 小于{{0.max}}个字节',
      isCreditCard: '无效',
      ...
      isLength: '长度应大于{{0.min}}个字, 小于{{0.max}}个字',
      isLowercase: '不是小写的',
